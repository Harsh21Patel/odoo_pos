...
export default function PaymentScreen() {
  const {
    ...
    confirmPayment, setView, confirmPaymentSuccess
  } = usePOSStore();
  ...
  const handlePay = async () => {
    ...
    try {
      if (paymentMethod === 'digital' || paymentMethod === 'upi') {
        ...
        const res = await paymentAPI.createRazorpayOrder({ orderId: currentOrder._id });
        ...
        const options = {
          ...
          name: "Odoo POS Cafe",
          description: `Order #${currentOrder?.orderNumber}`,
          ...
          handler: async function (response) {
            try {
              ...
              confirmPaymentSuccess(verifyRes.data);
            } catch (err) {
        ...
        await confirmPayment(parseFloat(cashReceived));
        toast.success('Payment successful!');
        setLoading(false);
      }
    } catch (e) {
      ...
      <div className="w-80 bg-white border-l border-gray-200 p-6 flex flex-col">
        ...
        {showUPI && (
          <div className="mt-4 p-4 bg-gray-50 rounded-xl border border-gray-200 text-center">
            <p className="text-xs text-gray-500 mb-2">Click "Confirm Payment" to open Razorpay Modal and pay ₹{total?.toFixed(2)}</p>
          </div>
        )}
